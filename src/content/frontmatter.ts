...
/**
 * Frontmatter here is flat `key: value` lines, so a YAML parser would be dead
 * weight. The build's page prerenderer reads the same posts from disk, so this
 * parser lives on its own rather than inside the browser-only loader.
 */
...
export const byNewestFirst = (a: Post, b: Post) => b.date.localeCompare(a.date);
